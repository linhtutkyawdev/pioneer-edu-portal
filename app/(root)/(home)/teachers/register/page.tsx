  ...
  if (!userId)
    return (
      <main className="text-gray-100 text-sm flex items-center justify-center h-80">
        ...
        This page is forbidden. You need to login first to register as a
        teacher.
      </main>
    );
    ...
    return (
      <section className="py-6 flex flex-col items-center">
        ...
        <h3 className="text-lg font-bold pb-6">
          Your regestration status is <b>"{applicant[0].status}".</b>
        </h3>
        <Profile id={userId} title="title" />
        {'rejected' == applicant[0].status && <ReRegisterButton id={userId} />}
      </section>
    );
  }
